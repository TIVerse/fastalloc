...
impl StatisticsReporter {
    ...
    pub fn as_key_value_pairs(&self) -> Vec<(&'static str, String)> {
        alloc::vec![
            ...
            (
                "utilization_rate",
                alloc::format!("{:.2}%", self.stats.utilization_rate())
            ),
            (
                "total_allocations",
                self.stats.total_allocations.to_string()
            ),
            (
                "total_deallocations",
                self.stats.total_deallocations.to_string()
            ),
            (
                "allocation_failures",
                self.stats.allocation_failures.to_string()
            ),
