...
impl<T> PoolConfigBuilder<T> {
    ...
    pub fn build(self) -> Result<PoolConfig<T>> {
        ...
        let capacity = self.capacity.ok_or_else(|| {
            Error::invalid_config("capacity must be specified")
        })?;
        ...
        let initialization_strategy = if self.pre_initialize && self.initialization_strategy.is_lazy() {
