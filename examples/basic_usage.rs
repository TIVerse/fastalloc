...
fn main() {
    ...
    *handle1 = 100;
    ...
    drop(handle1);
    drop(handle2);
    println!("   After dropping handles, available: {}\n", pool.available());
    ...
    let mut point = point_pool.allocate(Point { x: 10.0, y: 20.0 })
    ...
    
    {
        ...
        println!("   Allocated 2 strings, available: {}", reuse_pool.available());
        drop(h1); // Explicitly drop to show reuse
    }
    
    println!("   After dropping first handle, available: {}", reuse_pool.available());
    
    let h3 = reuse_pool.allocate(String::from("Third")).unwrap();
    println!("   Allocated new string (reused slot), available: {}", reuse_pool.available());
